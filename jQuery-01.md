...
> 移动端的zepto
>
> 这些库都是对原生 JavaScript 的封装，内部都是用JavaScript 实现的
>
...
> 链式编程、隐式遍历
>
> 对跨浏览器兼容。基本兼容了现在主流的浏览器
>
> 事件、样式、动画支持，大大简化了DOM操作
>
...
jQuery：javascript库，（下载）先要引入，在去用
...
​	jQuery对象：用jQeury获取的元素就是JQuery对象

​	DOM对象：用document.querySelector获取的元素DOM对象
...
​			  筛选方法：parent()【parsents()】,children()，find()，siblings()，eq(索引值)

隐式迭代：获取的jQuery存在伪数组，会遍历里面的DOM
